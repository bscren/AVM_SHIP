...
from utils.path_manager import get_yaml_dir, get_images_dir
...
def main():
    ...
    parser.add_argument('--camera_name', type=str, required=False,
                        default = 'right_front',choices=['front', 'back', 'left_front', 'left_back', 'right_front', 'right_back'],
                        help='相机名称')
    ...
    image_path = os.path.join(args.images_dir, f"cam_{args.camera_name}.jpg")
    calib_path = os.path.join(args.yaml_dir, f"cam_{args.camera_name}.yaml")
    prior_parameters_path = os.path.join(args.yaml_dir, "prior_parameters.yaml")
    # 检查图像文件
    ...
    # 初始化参数设置（使用统一路径管理）
    param_settings = ParamSettings(args.images_dir, args.yaml_dir)
    ...
    if os.path.exists(calib_path):
        try:
            param_settings.load_camera_calibration(args.camera_name, calib_path)
    ...
    # -----------------------------------------------------------------------------------
    # 加载监控范围的先验投影参数
    if prior_parameters_path:
        if not os.path.exists(prior_parameters_path):
            print(f"警告: 先验参数文件不存在: {prior_parameters_path}")
        else:
            try:
                param_settings.load_prior_projection_parameters(prior_parameters_path)
                print("已加载先验投影参数")
            except Exception as e:
                print(f"加载先验参数失败: {e}")
    # -----------------------------------------------------------------------------------
    ...
    mask_choice = input("是否为投影结果图像创建不规则多边形掩码？(y绘制/n不绘制/f绘制一个全白，默认f绘制一个全白): ").strip().lower()
    if mask_choice == '' or mask_choice == 'f':
        print("\n创建全白掩码")
        mapper.mask = np.ones((projected_image.shape[0], projected_image.shape[1]), dtype=np.uint8) * 255
        mapper.save_mask(args.images_dir)
        gui.show_result(mapper.mask, "Full White Mask")
        print("\n按任意键查看结果，按 'q' 退出")
        while True:
            key = gui.wait_key(30) # 30ms表示刷新间隔
            if key == ord('q'):
